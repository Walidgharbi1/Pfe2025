...
import CandidaturesCandidat from './CandidaturesCandidat';  // Import de la page Candidatures
...
export default function CandidatDashboard() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const fetchCandidatures = async () => {
        try {
          const response = await axios.get(`http://localhost:3000/candidatures/${user._id}`);
  ...
  const handlePostuler = async (offreId) => {
    try {
      await axios.post(`http://localhost:3000/candidatures`, { candidatId: user._id, offreId });
    ...
    <>
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 p-6 bg-gray-100">
          <h2 className="text-2xl font-bold mb-4">Bienvenue, {user ? user.nom : 'Candidat'}</h2>

          {/* Routes dynamiques */}
          <Routes>
            <Route path="profil" element={<ProfilCandidat user={user} />} />
            <Route path="candidatures" element={<CandidaturesCandidat candidatures={candidatures} />} />
